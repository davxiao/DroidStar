...
DroidStar::DroidStar(QObject *parent)
    : QObject(parent), m_dmrid(0), m_essid(0), m_dmr_destid(0), m_outlevel(0),
      m_mdirect(false), m_tts(0) {
...
void DroidStar::discover_devices() {
  ...
  m_vocoders.append("md380_vocoder");
  ...
  connect(manager, &QNetworkAccessManager::finished, this,
          [=](QNetworkReply *reply) {
            if (reply->error() == QNetworkReply::NoError) {
              QUrl url("https://www.allstarlink.org/portal/"
                       "webtransceiver.php?node=12345");
              QNetworkRequest request(url);
...
void DroidStar::process_connect() {
  if (connect_status != Mode::DISCONNECTED) {
    ...
    QJniObject::callStaticMethod<void>(
        "org/dudetronics/droidstar/NotificationClient", "denotify",
    ...
    if (m_protocol == "REF") {
      ...
      if ((m_hostmap[m_refname].contains(".nodes.allstarlink.org")) &&
          (m_wt_callingname.isEmpty() ||
           (m_asl_password != m_wt_callingname_pass))) {
        obtain_asl_wt_creds();
        return;
      }
    }
    ...
    if ((m_vocoder != "None") && (m_vocoder != "md380_vocoder") &&
        (m_vocoder.contains(':'))) {
    ...
    if (m_protocol == "IAX") {
      ...
      m_mode->set_iax_params(iaxuser, iaxpass, m_wt_callingname, m_refname,
                             m_host, m_port);
    ...
    // Allow modes to request the main app to toggle the connect button
    // (simulate user)
    connect(m_mode, SIGNAL(request_connect_toggle()), this,
            SLOT(process_connect()));
    connect(m_mode, SIGNAL(request_reconnect(int)), this,
            SLOT(schedule_reconnect(int)));
    ...
    if (m_protocol == "M17") {
      ...
      connect(this, SIGNAL(m17_send_sms(QString)), m_mode,
              SLOT(tx_packet(QString)));
      if (m_mdirect) {
  ...
  /*
          ...
          qDebug() << "process_connect called m_bm_password == " <<
     m_bm_password; qDebug() << "process_connect called m_tgif_password == " <<
  ...
  for (const auto &i : std::as_const(m_customhosts)) {
    ...
    if (line.at(0) == "IAX") {
      if (line.at(2).simplified() == "wt") {
        m_hostmap[line.at(1).simplified()] =
            line.at(1).simplified() + ".nodes.allstarlink.org," +
            line.at(3).simplified() + "," + line.at(4).simplified() + "," +
            line.at(5).simplified();
      } else {
        m_hostmap[line.at(1).simplified()] =
            line.at(2).simplified() + "," + line.at(3).simplified() + "," +
...
#if defined(Q_OS_ANDROID)
  QString vocname = "/vocoder_plugin.android.arm64";
  QString vocPath =
      QStandardPaths::writableLocation(QStandardPaths::AppDataLocation) +
      vocname;
  QFileInfo vocInfo(vocPath);
  if (!vocInfo.exists() || m_update_host_files) {
    QFile assetsFile("assets:/vocoder_plugin.android.arm64");
    if (assetsFile.exists()) {
      if (QFile::exists(vocPath))
        QFile::remove(vocPath);
      if (assetsFile.copy(vocPath)) {
        qDebug() << "Copied plugin from assets to " << vocPath;
        QFile::setPermissions(vocPath, QFile::ReadOwner | QFile::WriteOwner |
                                           QFile::ExeOwner | QFile::ReadUser |
                                           QFile::WriteUser | QFile::ExeUser |
                                           QFile::ReadGroup | QFile::ExeGroup |
                                           QFile::ReadOther | QFile::ExeOther);
      } else {
        qDebug() << "Failed to copy plugin from assets to " << vocPath;
      }
    } else {
      qDebug() << "Plugin not found in assets";
    }
  }
#endif
}
...
void DroidStar::update_data(Mode::MODEINFO info) {
  ...
  if ((connect_status == Mode::CONNECTING) &&
    ...
    QString s = "Connected to " + m_protocol + " " + m_refname + " " + m_host +
    ...
    QJniObject::callStaticMethod<void>(
        "org/dudetronics/droidstar/NotificationClient", "notify",
